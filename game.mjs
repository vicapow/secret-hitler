...
export default function update(
  ...
  if (message.type === "START_GAME") {
    ...
    const player = game.players.find(
      (player) => player.id === message.body.playerId
    );
    if (player) {
      // NOOP
    } else {
    ...
    const notVoted = game.players.filter(
    ...
    // if (isOver(game) && now - game.phase.timestamp > 10000) {
    //   game = freshGame(now);
    // }
    if (
      game.phase.name === "REVEAL_TICKET_RESULTS" &&
      now - game.phase.timestamp > 10000 //wait 10 seconds
    ) {
      ...
      if (win) {
        ...
        if (
          game.chancellorCandidate === game.hitler &&
          ...
          game = {
            ...game,
            phase: {
              name: "FASCISTS_WIN_WITH_HITLER_CHANCELLOR",
              timestamp: now,
            },
        ...
        let newFailedCount = game.failedVotes + 1;

        if (newFailedCount >= 3) {
          console.log("failed +3");
          let { policies } = game.policies.reduce(
            (acc, p) => {
              if (acc.found === false && p.location === "deck") {
                p.location = p.type;
                acc.found = true;
              }
              acc.policies.push(p);
      ...
      game.phase.name === "REVEAL_NEW_POLICY" &&
      ...
      game.phase.name === "REVEAL_POLICIES" &&
      now - game.phase.timestamp > 4000
    ) {
        ...
        if (fascistPolicies === 6) {
          ...
          if (fascistPolicies === 2) {
            game = {
              ...game,
              phase: {
                name: "PRESIDENT_INVESTIGATE_IDENTITY_START",
                timestamp: now,
              },
            };
    ...
    const index = game.policies.findIndex(
      (policy) => policy.id === message.body.policyId
    );
    const [discarded, policies] /*: [Policy, $ReadOnlyArray<Policy>] */ = pluck(
      game.policies,
      index
    );
    game = {
  ...
  } else if (message.type === "EXAMINING_PLAYER") {
    game = {
      ...game,
      examinePlayer: message.body.playerId,
    };
  } else if (message.type === "NOMINATE_PLAYER") {
    game = startNextElection(game, now, message.body.playerId)
  } else if (message.type === "DONE_EXAMINING_PLAYER") {
    game = {
      ...game,
      examinePlayer: null,
    };
...
function startNextElection(
  ...
  let candidateId, resumeId;
  if (game.electionResumeAt) {
    candidateId = game.electionResumeAt;
  } else {
    const nextInLine = playerRight(
      game.players.filter((player) => !player.killed),
      (player) => player.id === game.electedPresident
    ).id;
    if (specialId) {
      candidateId = specialId;
      resumeId = nextInLine;
    } else {
      candidateId = nextInLine;
    }
  }
  ...
  return {
    ...
    presidentCandidate: candidateId,
    electionResumeAt: resumeId,
    // presidentCandidate
  };
}
